...
WESTOCK_SCRIPT = r"C:\Users\warning5\.workbuddy\plugins\marketplaces\cb_teams_marketplace\plugins\finance-data\skills\westock-data\scripts\index.js"
NODE_EXE = r"C:\Users\warning5\.workbuddy\binaries\node\versions\22.22.2\node.exe"
...
def query_westock(codes: list, start_date: str, end_date: str) -> str | None:
    ...
    try:
        r = subprocess.run(
            [NODE_EXE, WESTOCK_SCRIPT, "asfund", code_arg,
             "--start", start_date, "--end", end_date],
            capture_output=True, timeout=180,
            cwd=r"C:\Users\warning5\.workbuddy\plugins\marketplaces\cb_teams_marketplace\plugins\finance-data\skills\westock-data\scripts",
        )
        if r.returncode != 0:
...
def extract_westock_moneyflow(md_text: str, ws_code_set: set = None) -> dict:
    ...
    i_block     = _idx("BlockNetFlow")    # 超大单净流入
    i_jumbo     = _idx("JumboNetFlow")   # 大单净流入
