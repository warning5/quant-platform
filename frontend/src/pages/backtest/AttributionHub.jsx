  ...
  const suggestions = useMemo(() => {
    ...
    if (worstLoss && worstLoss.contributionPct < -30) {
      tips.push({
        ...
        path: '步骤1：策略管理 → 策略列表 → 编辑策略 → 调仓频率改为 WEEKLY → 回测验证\n步骤2：若胜率仍 <40%，因子管理 → IC管理 → 标出 IC<0.02 或 IR<0.3 的因子\n步骤3：策略管理 → 策略列表 → 编辑策略 → 因子配置 Tab → 降低或移除这些因子',
        limitation: '⚠️ (1) 因子 IC 值不在策略编辑页中，需到「因子管理 → IC管理」查看；(2) 策略编辑器只显示因子权重，不能直接看到 IC，需手动交叉对照。',
        ...
        action: worstLoss.winRate < 0.3
          ? '调仓频率 DAILY→WEEKLY 是最快可执行的验证手段。如果改后短周期胜率仍低于 40%，则需到「因子管理 → IC管理」查看各因子 Rank IC 均值，标出 IC<0.02 或 IR<0.3 的因子，再到「策略编辑 → 因子配置 Tab」降低或移除。' +
    ...
    if (losingRatio > 0.6) {
      tips.push({
        ...
        path: '步骤1：因子管理 → IC管理 → 选因子+日期 → 点"分析" → 在结果表格中点击蓝色因子代码，下方展开 IC 趋势图 → 看累计 IC 曲线是持续下降（长期失效）还是近期才掉（暂时波动）\n步骤2：检查该因子与备选因子的相关系数，确认有低相关（<0.3）的替代品才值得替换\n步骤3：策略管理 → 模拟盘 → 新建副本策略 → 修改因子配置 → 跑同期回测 A/B 对比 → 夏普/回撤都改善再正式替换',
        limitation: '⚠️ IC低≠去掉就好：该因子可能在组合中起噪声抵消或分散化作用，去掉反而更差。系统暂无自动 A/B 对比功能，需手动在模拟盘建副本验证。IC管理中的因子趋势图可辅助判断是否真的失效。',
