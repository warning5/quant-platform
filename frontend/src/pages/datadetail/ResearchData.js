...
function ResearchData() {
  ...
  const reportColumns = [
    ...
    { title: <Tooltip title={epsTip} overlayClassName="tip-light" overlayStyle={{ maxWidth: 420 }}><span style={{ cursor: 'pointer' }}>EPS预测<QuestionCircleOutlined style={{ fontSize: 12, marginLeft: 4 }} /></span></Tooltip>, key: 'eps', width: 220, align: 'right',
      render: (_, record) => {
    ...
    { title: <Tooltip title={peTip} overlayClassName="tip-light" overlayStyle={{ maxWidth: 420 }}><span style={{ cursor: 'pointer' }}>PE预测<QuestionCircleOutlined style={{ fontSize: 12, marginLeft: 4 }} /></span></Tooltip>, key: 'pe', width: 220, align: 'right',
      render: (_, record) => {
        const f = record.epsForecast;
        if (!f || Object.keys(f).length === 0) return '-';
        const years = Object.keys(f).sort();
