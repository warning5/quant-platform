...
public class TradingSignalEngine {
    ...
    public List<ScoreRule> getScoreRules() {
        ...
        rules.add(new ScoreRule("资金面", MONEY_WEIGHT,
                "主力净流入(5分)：≥5亿=5分, ≥1亿=4分, >0=3分, >-1亿=1分, ≤-1亿=0分\n" +
                "主力净流入占比(5分)：≥10%=5分, ≥5%=4分, >0%=3分, >-5%=1分, ≤-5%=0分\n" +
                "融资余额变化(3分)：>5%=3分, >2%=2分, >0%=1分, ≤0%=0分\n" +
                "5日累计主力净流入(3分)：≥3亿=3分, ≥1亿=2分, >0=1分, ≤0=0分\n" +
                "股东人数变化(2分)：减少>10%=2分(筹码高度集中), 减少>5%=1分, 其余=0分\n" +
                "量比(2分)：≥2.0=2分, ≥1.5=1分, <1.5=0分\n" +
                "换手率偏离(3分)：>0%=3分, >-2%=2分, ≤-2%=1分\n" +
                "内外盘比(2分)：>1.2=2分(强势买方), >1.0=1分, ≤1.0=0分",
                "当日主力净流入(东财资金流) + 融资余额(最新两期) + 股东人数(最新两期) "
                        + "+ 量比(当日/近5日均量) + 换手率偏离(当日-近20日均) + 内外盘比(腾讯实时盘口快照)"));

        rules.add(new ScoreRule("资金面·数据说明", 0,
                "内外盘比依赖腾讯实时盘口快照(qt.gtimg.cn)，仅返回当日累计值，不支持历史回溯；"
                        + "历史未采集的交易日无法补采，在内外盘面板中显式标注为“无数据”，均值仅按有效交易日计算，不做插值。\n"
                        + "内外盘比与主力净流入相关性较低(Spearman≈0.2，约4成样本方向相反)，"
                        + "反映的是盘口主动买卖而非大单资金净额，属独立信号：涨停/跌停时比值失真，需结合股价位置判断。",
                "-"));
