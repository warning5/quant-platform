...
public class DataPermissionService {
    ...
    public List<ResourceOptionVO> listOptions(String type) {
        ...
        return switch (rt) {
            case STRATEGY -> {
                List<StrategyDefinition> rows = strategyMapper.selectList(
                        new QueryWrapper<StrategyDefinition>().select("id", "strategy_code", "strategy_name")
                                .last("ORDER BY id DESC LIMIT 200"));
                yield rows.stream().map(r -> new ResourceOptionVO(r.getId(),
                        r.getStrategyCode() + " - " + (r.getStrategyName() == null ? "" : r.getStrategyName())))
                        .toList();
            }
            case FACTOR -> {
                List<FactorDefinition> rows = factorMapper.selectList(
                        new QueryWrapper<FactorDefinition>().select("id", "factor_code", "factor_name")
                                .last("ORDER BY id DESC LIMIT 200"));
                yield rows.stream().map(r -> new ResourceOptionVO(r.getId(),
            ...
            case BACKTEST -> {
                List<BacktestTask> rows = backtestMapper.selectList(
                        new QueryWrapper<BacktestTask>().select("id", "task_name", "strategy_code")
                                .last("ORDER BY id DESC LIMIT 200"));
                yield rows.stream().map(r -> new ResourceOptionVO(r.getId(),
            ...
            case PAPER_TRADING -> {
                List<PaperTrading> rows = paperMapper.selectList(
                        new QueryWrapper<PaperTrading>().select("id", "strategy_code")
                                .last("ORDER BY id DESC LIMIT 200"));
                yield rows.stream().map(r -> new ResourceOptionVO(r.getId(),
                        "模拟盘 #" + r.getId()
                                + (r.getStrategyCode() != null ? "（策略:" + r.getStrategyCode() + "）" : "")))
