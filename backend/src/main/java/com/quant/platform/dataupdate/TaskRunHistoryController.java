...
public class TaskRunHistoryController {
    ...
    @GetMapping("/sla")
    @Operation(summary = "SLA 监控看板：各核心任务今日执行状态 vs SLA 期望")
    public ApiResponse<List<Map<String, Object>>> slaDashboard() {
        try {
            // 取 sla_config 中启用项，关联今日最近一次执行记录
            // 用 AS 别名统一输出 camelCase，与前端字段命名一致
            List<Map<String, Object>> rows = jdbcTemplate.queryForList(
                "SELECT s.task_key AS taskKey, s.task_name AS taskName, " +
                "s.expected_finish_hour AS expectedFinishHour, s.max_duration_min AS maxDurationMin, " +
                "s.severity AS severity " +
                "FROM sla_config s WHERE s.enabled = 1 ORDER BY " +
                "CASE s.severity WHEN 'HIGH' THEN 1 WHEN 'MEDIUM' THEN 2 ELSE 3 END, s.task_key");
